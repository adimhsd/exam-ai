...
# Buat tabel database secara otomatis (untuk lingkungan dev/MVP)
# Pastikan db siap sebelum memproses request
max_retries = 5
for i in range(max_retries):
    try:
        Base.metadata.create_all(bind=engine)
        print("Tabel basis data berhasil diinisialisasi.")
        break
    except Exception as e:
        print(f"Percobaan inisialisasi basis data gagal ({i+1}/{max_retries}): {e}")
        time.sleep(3)
